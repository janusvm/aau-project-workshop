...
\begin{frame}[containsverbatim,label=dirs]{Mappestruktur og Filtyper}{Mappestruktur}
...
\begin{frame}[containsverbatim]{Opgaver}{Opgave 1}
  \begin{itemize}
  \item Lav en AAU mappestruktur, f.eks. som vist på slide \ref{dirs}
  \item Lav en passende undermappe til dette kursus
  \item Opret her filen \texttt{hello\_world.tex}
  \item Skriv følgende i filen og kompiler dokumentet:
  \end{itemize}

  \begin{block}{hello\_world.tex}
\begin{verbatim}
  \documentclass[10pt]{article}
  \begin{document}
   Hello World! I $\heartsuit$ Mathematics!
  \end{document}
\end{verbatim}
  \end{block}
...
\begin{frame}[containsverbatim]{Opgaver}{Opgave 2}
  \begin{itemize}
  \item Lav en pdf med nedenstående indhold:
    \footnote{Hjælp kan findes på: \url{https://en.wikibooks.org/wiki/LaTeX/Mathematics}\\}
  \end{itemize}
  \begin{align*}
    \pi                               & = \frac{O}{D}    \\
    e^{i\pi} + 1                      & = 0              \\
    \frac{\partial^2 u}{\partial t^2} & = c^2 \nabla^2 u \\
    P_k(x)                            & = \sum_{i=0}^{k} \frac{f^{(k)}(a)}{k!}(x - a)^k
  \end{align*}
\end{frame}
